...
#food_detail_table tr{
	height:20px;
}
#food_detail_table td:nth-child(odd){
	padding-left:20px;
	text-align:right;
}
#food_detail_table td:nth-child(even) input,select{
	width:400px;
}
</style>
	<form id="food_detail_form">
		<input type="hidden" name="id" value="${ bean.id }"/>
		<table id="food_detail_table">
			<tr>
				<td>饵料名称：</td>
				<td>
					<input disabled class="easyui-textbox" id="food_detail_name" name="name" value="${ bean.name }"/>
				</td>
			</tr>
			<tr>
				<td>适宜鱼种：</td>
...
$(function(){
	//所有select下拉的初始化
	$("#food_detail_form select").each(function(i){
