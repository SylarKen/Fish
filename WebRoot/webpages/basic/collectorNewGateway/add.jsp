...
<body>
<form id="collectorNewGate_add_form" method="post" action='${basePath}collectorNewGateway/collectorNewGate_add'>
...
<%--		</tr>--%>
		<tr style="height:40px;">
			...
			<td>
				<input id="collector_add_deptName" name="deptName"  style="width:210px;">
				<input id="collector_add_deptCode" name="deptCode" type="hidden">
			</td>
		</tr>
		<tr style="height:40px;">
	...
	$('#collector_add_areaName').textbox({
        ...
        onClickButton:function(){
      	 	showSelectAreaDialog($("#collector_add_areaCode").val(),function(row){
				...
				$("#collector_add_deptName").textbox('clear');
				$("#collector_add_deptCode").val('');
	        });
        }
    });

	//选择池塘对话框
	$('#collector_add_deptName').textbox({
		editable: false,
		width:210,
		buttonText:'查询',
		iconAlign:'right',
		...
		onClickButton:function(){
			...
			showSelectPondDialog(areaCode,function(row){
				$("#collector_add_deptName").textbox('setValue',row.name);
				$("#collector_add_deptCode").val(row.code);
			});
		}
	});
